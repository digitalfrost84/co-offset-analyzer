...
async function analyzeFixture(api, fixture) {
  ...
  const validRows = await api.filterRowsByCurrentAsync(data.rows, loadSensor, loadThreshold.current, analysisId);
  if (validRows.length === 0) throw new Error(`${fixture}: no rows passed load threshold`);
  ...
  const { referenceMeans, groupMeans } = api.calculateReferenceMeans(vidMeans, groups);
  const vidStats = api.calculateVidStats(vidMeans, referenceMeans);
  ...
  const recommendations = api.calculateOffsets(vidMeans, referenceMeans, 3.0, currentOffsets, vidStats);
  ...
  return {
    ...
    groupMeans: roundMap(groupMeans),
